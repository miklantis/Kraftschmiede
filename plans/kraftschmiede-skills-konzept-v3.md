# Kraftschmiede – Skills-System: Konzept v3

Konsolidierte Neufassung nach dem Grilling. Löst v2 ab. Gegen den echten Code
geprüft (app.js, data.js, engine.js, live.js, index.html, Stand Schema 0.14).
...
|---|---|---|
| `app.js` | Schema-Konstante (`"0.14"`), geteilte Helfer, Views, Nav (`TABS`), Settings, Journey-Logik | Equipment-Inventar-UI, **neuer Skills-Tab**, Skill-Kachel im Training, Kalender-/Verlauf-Zweig |
...
| `live.js` | Live-Session, Timer, Audio, Rest-Bar, Start-/Ende-Dialoge | **Skill-Live-Screen** + Abschluss als zweiter Renderpfad neben dem Workout |
...
unverändert aus der vorhandenen Infrastruktur. Die Set-Rows richten sich nach `metric`
(Abschnitt 7). Die Langhantel-Felder (Gewicht, Plate-Hint, Vorschlag) entfallen.
...
aus live.js (`playBeep`, `buzz`, `clickTick`, Rest-Bar-Mechanik) – kein neuer Apparat.
...
5. **live.js + app.js** – Skill-Kachel im Training (Auswahl + Auswahl-Tor),
   ...
   Timer-Infrastruktur), Start-/Ende-Dialog wiederverwenden, `finishSkillSession`
   (`type:"skill"`-Zeile + `skillProgress`-Fortschreibung).
