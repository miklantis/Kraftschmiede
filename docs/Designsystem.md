...
| **Section** | Abschnitt mit kleiner, gesperrter Versal-Eyebrow plus Inhalt. Auf fast jeder Seite. Optional sitzt rechts neben der Überschrift eine Aktion (`action`), typischerweise ein Icon-Knopf – der Baustein gibt nur Platz und Ausrichtung vor, den Inhalt liefert der Aufrufer (heute der Stift am Journey-Workout-Abschnitt). |
...
| **Overlay** | Popup-Fundament für alle modalen Dialoge: Desktop zentriertes Fenster, Mobile Bodenblatt von unten. Fährt am Handy die Bildschirmtastatur hoch, endet die Schicht an deren Oberkante (`useKeyboardInset`) – sonst reicht sie darunter weiter, weil iOS das Layout-Fenster nicht verkleinert, und der Inhalt unter dem getippten Feld verschwindet hinter der Tastatur. Darauf setzt u. a. das bereichsübergreifend genutzte „Was ist neu"-Popup `WhatsNewSheet` (Trainingsseite + Einstellungen) auf. Optional teilt sich der Inhalt die Blatthöhe selbst auf (`contentScrolls`): Dann scrollt nicht mehr das ganze Blatt, sondern nur der Bereich, den der Aufrufer dafür auszeichnet (`min-h-0 overflow-y-auto`) – Kopf und Fußknopf bleiben stehen. Solange die Tastatur offen ist, behält das Blatt in diesem Modus eine feste Höhe, damit es beim Tippen nicht mit dem Inhalt springt. Für Dialoge mit Suchfeld über einer Liste, heute das Workout-Auswahl-Popup; Standard ist aus, alle übrigen Dialoge bleiben wie sie sind. |
...
| **SearchField** | Einzeiliges Suchfeld über einer Liste: Lupe links, Eingabefeld, Löschen-x rechts sobald etwas drinsteht (Löschen setzt zurück und gibt den Fokus ans Feld). Domänenfrei – Wert, Änderungs-Callback, Platzhalter und aria-label kommen vom Aufrufer; gefiltert wird nicht hier, sondern in der Aufbereitungslogik (`lib/`). **Verbindlich für neue Suchfelder**, heute im Workout-Auswahl-Popup der Journey-Seite, ganz oben auf der Übungen-Seite (dort nicht klebend, über den angehefteten Diagrammen; gefiltert wird mit `filterExercises` vor dem Gruppieren) und ganz oben auf der Workouts-Seite (ebenfalls nicht klebend, erst ab `SUCHE_AB_WORKOUTS` Workouts sichtbar – die Schwelle teilt sich die Seite mit dem Auswahl-Popup; gefiltert wird mit `filterWorkoutRows` über die eine Liste, die die Seite seit Issue #491 noch hat). Der `ExercisePicker` hat sein Suchfeld noch handgebaut (gleiche Optik); die Übernahme dort ist ein eigenes Vorhaben. |
...
| **LoadMore** | Nachladen bei gekürzten Listen: kein Rahmen, kein Hintergrund, kein Text – nur ein dezent grauer Chevron nach unten, zentriert über die volle Breite als Tippfläche (Beschriftung nur als `aria-label`). **Verbindlich für alle Listen**, die zunächst einen Teil zeigen und nachladen können; kein eigener Outline-Button mehr. Den Zähler dahinter hält der Hook `useMehrLaden` (sichtbarer Ausschnitt, Rest-Flag, Nachladen; Seitengröße standardmäßig fünf) – zusammen genutzt in der Verlauf-Liste, im Befinden-Verlauf, bei den Messungen und den Zeiträumen. Die Messungen zeigen abweichend zunächst drei Einträge und laden in Dreierschritten nach. |
...
| **ChipList** | Wertereihe als Chips, reine Anzeige: dieselbe Pille wie im `ChipEditor` (`CHIP_BASE`, dort mitbenutzt), aber ohne × und ohne „+ Wert"-Knöpfe. Für Orte, die einen Bestand nur zeigen – heute die Gewichts-Reihen im Inventar-Abschnitt der Übungsseite, damit Übungsseite und Einstellungen dieselben Werte gleich zeigen. Rendert ein `span` mit `display:flex`, steht deshalb auch in der Unterzeile einer Listenreihe (`SettingRow`). Die Einheit gehört in die Beschriftung, nicht an jeden Wert. |
