...
| **PhaseBar** | Segmentbalken für den Phasen-Stand eines Skills: ein Segment je Phase über die volle Breite, erledigte gefüllt (Skill-Farbe gedeckt), die aktuelle kräftig, künftige blass; gemeistert = alle gefüllt. Auf der Trainingsseite in der Skill-Liste (als `footer` der Listenzeile) und im Kopf der Skill-Karte. Bewusst andere Optik als ProgressDots, die für Wocheneinheiten stehen. |
