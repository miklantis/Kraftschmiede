...
| **SearchField** | Einzeiliges Suchfeld über einer Liste: Lupe links, Eingabefeld, Löschen-x rechts sobald etwas drinsteht (Löschen setzt zurück und gibt den Fokus ans Feld). Domänenfrei – Wert, Änderungs-Callback, Platzhalter und aria-label kommen vom Aufrufer; gefiltert wird nicht hier, sondern in der Aufbereitungslogik (`lib/`). **Verbindlich für neue Suchfelder**, heute im Workout-Auswahl-Popup der Journey-Seite, ganz oben auf der Übungen-Seite (dort nicht klebend, über den angehefteten Diagrammen; gefiltert wird mit `filterExercises` vor dem Gruppieren) und ganz oben auf der Workouts-Seite (ebenfalls nicht klebend, erst ab `SUCHE_AB_WORKOUTS` aktiven Workouts sichtbar – die Schwelle teilt sich die Seite mit dem Auswahl-Popup; gefiltert wird mit `filterWorkoutRows`, aktive Liste und Archiv getrennt). Der `ExercisePicker` hat sein Suchfeld noch handgebaut (gleiche Optik); die Übernahme dort ist ein eigenes Vorhaben. |
