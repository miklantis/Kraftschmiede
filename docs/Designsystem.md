...
| **SearchField** | Einzeiliges Suchfeld über einer Liste: Lupe links, Eingabefeld, Löschen-x rechts sobald etwas drinsteht (Löschen setzt zurück und gibt den Fokus ans Feld). Domänenfrei – Wert, Änderungs-Callback, Platzhalter und aria-label kommen vom Aufrufer; gefiltert wird nicht hier, sondern in der Aufbereitungslogik (`lib/`). **Verbindlich für neue Suchfelder**, erstmals im Workout-Auswahl-Popup der Journey-Seite. Der `ExercisePicker` hat sein Suchfeld noch handgebaut (gleiche Optik); die Übernahme dort ist ein eigenes Vorhaben. |
