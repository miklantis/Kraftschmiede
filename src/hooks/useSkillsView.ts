...
// Fuehrt Definitionen, Fortschritt und Equipment zu einer einzigen Skill-Liste
...
export function useSkillsView(): SkillsView {
  ...
  const cards: SkillCardView[] = skills.map((s) => {
    ...
    return {
      ...
      missingEquipment: adv.missingEquipment.map((k) => labelByKey.get(k) ?? k),
  ...
  return { isLoading, isError, error, skills: cards };
}
